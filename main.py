...
class CryptoTrackerApp(App):
    ...
    def rebuild_ui_layout_structures(self):
        """Builds components safely once—never loops clear_widgets on active inputs"""
    ...
    def add_custom_coin(self, instance):
        ...
        try:
            res = requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={target_pair}", timeout=3)
            if res.status_code == 200:
                self.fixed_pairs[target_pair] = float(res.json()['price'])
                self.known_matrix_coins.add(input_coin)
                self.rebuild_matrix_dictionary()
    ...
    def refresh_live_data_only(self, dt):
        """Updates ONLY text contents inside active labels, preventing frame layout crashes entirely"""
        try:              
            res = requests.get("https://api.binance.com/api/v3/ticker/price", timeout=2)
            if res.status_code != 200: return
            live_prices = {item['symbol']: float(item['price']) for item in res.json()}
        except:
            return
